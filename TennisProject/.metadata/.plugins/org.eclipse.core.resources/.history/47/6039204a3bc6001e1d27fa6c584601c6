...
public class GameMethod extends FileManager{
	...
	public int getSet(String gender) {
		...
		do {
			...
			//if
		} while (!gender.contains("남자") &&  !gender.contains("여자") &&  !gender.contains("혼성"));
		//do~while
		return selectSet;
	}

	public boolean getMode(String gender) {// 단식이면 true, 복식이면 false
	...
	public void plusSet(int p, int set) {
		...
		if (scores[p-1][2] >= (set/2+1)) {
			...
			System.out.println();
			System.out.println();
			System.out.println("\t\t\t\t\t최종 승리팀 : " + "Team" + p); // *******************
			System.out.println();
			System.out.println();
			System.out.println();
			System.out.println();
