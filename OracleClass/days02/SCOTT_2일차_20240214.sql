...
FROM emp;
-- emp 테이블의 job 컬럼만 조회
SELECT ALL job  -- ALL 출력할 컬럼이 중복이 되더라도 모두 출력 (
FROM emp;

-- DISTINCT    출력할 컬럼이 중복이 될 때 한 번만 출력 ( 암기 )
SELECT DISTINCT job
FROM emp;
...
FROM emp
WHERE deptno = 10;
WHERE deptno == 10;  --384행, 15열에서 오류 발생, ORA-00936: missing expression
--
...
-- WHERE 2000<= sal + NVL(comm, 0) AND sal + NVL(comm, 0) <= 4000  -- ORA-00904: "PAY": invalid identifier
