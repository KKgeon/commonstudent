...
--2. N+CHAR 고정길이  2000byte
--3. VAR+CHAR2 가변길이 4000byte
--4. N+VAR+CHAR2 가변길이 4000byte
--5. LONG       가변길이  2GB
...
(
    no      NUMBER(2)    NOT NULL  PRIMARY KEY --  PK==NN + UK유일성 제약조건
...
create table tbl_board (
   seq number not null  primary key -- PK=NN+UK
...
create global temporary table tbl_board2 (seq number not null,writer varchar2(20) not null ,password varchar2(15) not null ,title varchar2(100) not null,content clob,regdate date);
...
COMMIT;
-- ORA-00001: unique constraint (SCOTT.SYS_C007040) violated
--            유일성  제약조건                         위배된다.
INSERT INTO tbl_board (seq,writer,passwd,title,content,regdate)
...
COMMIT;
--            유일성  제약조건                         위배된다.
INSERT INTO tbl_board ( writer,seq,passwd,title,content, regdate)
...
-- , 제약조건이름을 지정하지 않으면 SYS_XXXX  이름으로 자동 부여된다. 
-- 제약조건이름 : SCOTT.SYS_C007040
SELECT *
...
ADD readed NUMBER DEFAULT 0;  -- 1개 컬럼만 추가할 경우 () 괄호 생략 가능하다.
-- ADD ();
DESC tbl_board;
--
SELECT *
...
--   ㄴ 이미 존재하는 테이블 이용해서 => 새로운 테이블 생성 + 데이타(레코드) 추가
--【형식】
...
-- 참고하고 있는 부모 테이블의 경우는 삭제 불가능

-- 예 ) 기존 테이블 -> 새로운 테이블 생성 + 레코드 X 기존 테이블의 구조만 복사하는 방법
...
-- JOIN 조건 : dept.deptno = emp.deptno (equal join)
--          : emp = salgrade (NON eqaul join) (BETWEEN AND)
-- dept     : deptno, dname
